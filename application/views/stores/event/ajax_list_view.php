...
<tr>
  ...
  <td class="hidden-480"><?php echo $row->detail; ?></td>
...
$(document).ready(function(){
  $("form#frmtrash a").click(function(){
    ...
    {
      var url = '<?php echo base_url(); ?>stores/event/update?id='+a.attr('dataid');
      $(location).attr('href',url);
    }                
    if($(this).attr("action") == "del")
    {                    
        $.ajax({
            url: '<?php echo base_url(); ?>stores/event/movetrash',
            type: "POST",
            dataType: 'json',
    ...
    {
    $.ajax({
            url: '<?php echo base_url(); ?>stores/event/status',
            type: "post",
            dataType: 'json',
