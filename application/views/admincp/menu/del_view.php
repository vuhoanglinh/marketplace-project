  ...
  <div class="container-fluid">
    ...
    <div class="breadcrumbs">
      <ul>
        ...
        <li> <a href="<?php echo base_url(); ?>admincp/menu/trash">Đã xóa</a> </li>
      </ul>
    ...
    <div class="row-fluid">
      <div class="span12">
        <div class="box box-color box-bordered">
          <div class="box-title">
            <h3> <i class="icon-table"></i> Danh sách menu đã xóa </h3>
          </div> 
          <script> 
          $(document).ready(function(){ 
           ...
           $('#search').keypress(function(event){
              if(event.which == 13) {
                if($('#search').val() != ""){
                  var url = '<?php echo base_url(); ?>admincp/menu/trash?key='+$('#search').val().trim();
          ...
          <div class="box-content nopadding"> <?php echo form_open("admincp/menu/delete", array('id' => 'frmdel', 'name' => 'frmdel'));?>
            <div class="dataTables_wrapper">         
              <div class="dataTables_length"><label><span style="margin: 0px;"><a href="javascript:void(0)" id="btnsearch" class="btn btn-primary"><i class="icon-search"></i></a></span>  <input type="text" id="search" placeholder="Tìm kiếm..."></label></div>         
              ...
              <div class="dataTables_length">
                ...
                <input type="hidden" id="hd_redirect" name="hd_redirect" value="<?php echo current_url(); ?>">
            ...
            <script type="text/javascript">              
              ...
              $(document).ready(function(){
                ...
                function loadmenu(page) {
                  $.ajax({
                      url: "<?php echo base_url('admincp/menu/pagging_menutrash'); ?>",
                      type: "post",
                    ...
                    $("input[type=checkbox]").each(function(){ 
                      if($(this).attr("id") != "check_all") 
                      {
                        if($(this).is(":checked"))
                        {
                          var form = $('frm#frmdel');
                          $.ajax({
                              url: "<?php echo base_url('admincp/menu/delete'); ?>",
                              type: "post",
                              data: "id="+$(this).val(),
                              beforeSend : function (){
                ...
                $("form#frmdel").on('submit', function(){                  
                  return false;
                });
                ...
                $("#btn_outtrash").on('click', function(){
                    $("input[type=checkbox]").each(function() { 
                      if($(this).attr("id") != "check_all") 
                      {
                        if($(this).is(":checked"))
                        {
                          var from = $("form#frmdel");

                          $.ajax({
                              url: "<?php echo base_url(); ?>admincp/menu/outtrash",
                              type: "POST",
                              dataType: "json",
                              data: 'id=' + $(this).val(),
                              beforeSend : function (){
