    ...
    <div class="row-fluid">
      <div class="span12">
        <div class="box box-color box-bordered">
          <div class="box-title">
            <h3> <i class="icon-table"></i> Danh mục email đã xóa </h3>
          </div> 
          <script> 
          $(document).ready(function(){ 
           ...
           $('#search').keypress(function(event){
              if(event.which == 13) {
                if($('#search').val() != ""){
                  var url = '<?php echo base_url(); ?>admincp/email/trashct?key='+$('#search').val().trim();
          ...
          <div class="box-content nopadding"> <?php echo form_open("admincp/email/delct", array('id' => 'frmdel', 'name' => 'frmdel'));?>
            <div class="dataTables_wrapper">  
              ...
              <div class="dataTables_length">
                ...
                <button type="submit" id="btn_delselect" name="btn_delselect" class="btn btn-inverse" >
              ...
              <?php echo form_close() ?>
              <div class="table-pagination"> <a href="#" class='disabled'>First</a> <a href="#" class='disabled'>Previous</a> <span> <a href="#" class='active'>1</a> <a href="#">2</a> <a href="#">3</a> <a href="#">4</a> <a href="#">5</a> </span> <a href="#">Next</a> <a href="#">Last</a> </div>
            </div>
            <script type="text/javascript">              

              $(document).ready(function(){

                $("form#frmdel a").click(function(){
                  var a = $(this); 
                  /*
                  ...
                  */
                  if($(this).attr("action") == "del")
                  {
                    if(confirm('Bạn có chắc chắn muốn xóa không? \nThao tác này sẽ xóa dữ liệu vĩnh viễn.') == true) { 
                      $.ajax({
                            url: '<?php echo base_url(); ?>admincp/email/deletect',
                            type: "POST",
                            dataType: 'json',
                  ...
                  {
                    $.ajax({
                      url : "<?php echo base_url(); ?>admincp/email/statusct",
                      type : "POST",
                      dataType : "json",
                ...
                $("#btn_outtrash").on('click', function(){
                    ...
                    var from = $("form#frmdel");
                      ...
                      $.ajax({
                          url: "<?php echo base_url(); ?>admincp/email/outtrashct",
                          type: "POST",
                          data: $(from).serialize(),
                          beforeSend : function (){
                            $("#modal-4 .modal-body p").html('Đang tải...');
