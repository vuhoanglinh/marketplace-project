...
$(document).ready(function() {
		...
		$('.dataTable').each(function(){
			if(!$(this).hasClass("dataTable-custom")) {
				...
				if($(this).hasClass('dataTable-tools')){
					opt.sDom= "T"+opt.sDom;
					opt.oTableTools = {
						"sSwfPath": "http://localhost:81/chon/public/js/plugins/datatable/swf/copy_csv_xls_pdf.swf"
					};
				}
				if($(this).hasClass("dataTable-scroller")){
					...
					opt.sAjaxSource = "http://localhost:81/chon/public/js/plugins/datatable/demo.txt";
				}
	...
	if($('.file-manager').length > 0)
	{
		$('.file-manager').elfinder({
			url:'http://localhost:81/chon/public/js/plugins/elfinder/php/connector.php'
		});
	}
